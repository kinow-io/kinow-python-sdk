...
class VideosApi(object):
    ...
    def attach_cover_to_video(self, video_id, id_image, **kwargs):
        ...
        :param int id_image: ID of the image to attach (required)
        :return: None
    ...
    def get_player_iframe(self, video_id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_player_iframe(video_id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_player_iframe_with_http_info(video_id, **kwargs)
        else:
            (data) = self.get_player_iframe_with_http_info(video_id, **kwargs)
            return data

    def get_player_iframe_with_http_info(self, video_id, **kwargs):
        """
        Get video player url
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_player_iframe_with_http_info(video_id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_player_iframe" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'video_id' is set
        if ('video_id' not in params) or (params['video_id'] is None):
            raise ValueError("Missing the required parameter `video_id` when calling `get_player_iframe`")


        collection_formats = {}

        resource_path = '/videos/{video_id}/player-iframe'.replace('{format}', 'json')
        path_params = {}
        if 'video_id' in params:
            path_params['video_id'] = params['video_id']
