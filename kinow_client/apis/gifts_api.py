...
class GiftsApi(object):
    ...
    def get_gift_by_token(self, token, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_gift_by_token(token, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str token: Gift token to fetch (required)
        :return: Gift
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_gift_by_token_with_http_info(token, **kwargs)
        else:
            (data) = self.get_gift_by_token_with_http_info(token, **kwargs)
            return data

    def get_gift_by_token_with_http_info(self, token, **kwargs):
        """
        Retrieve a Gift using its token
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_gift_by_token_with_http_info(token, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_gift_by_token" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'token' is set
        if ('token' not in params) or (params['token'] is None):
            raise ValueError("Missing the required parameter `token` when calling `get_gift_by_token`")


        collection_formats = {}

        resource_path = '/gifts/{token}'.replace('{format}', 'json')
        path_params = {}
        if 'token' in params:
