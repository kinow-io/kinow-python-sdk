...
class GroupsApi(object):
    ...
    def create_group(self, body, **kwargs):
        ...
        :param callback function: The callback function
            for asynchronous request. (optional)
        :param CreateGroupRequest body: Group settings (required)
    ...
    def get_group_customers(self, group_id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_group_customers(group_id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_group_customers_with_http_info(group_id, **kwargs)
        else:
            (data) = self.get_group_customers_with_http_info(group_id, **kwargs)
            return data

    def get_group_customers_with_http_info(self, group_id, **kwargs):
        """
        Get customers list linked to a group
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_group_customers_with_http_info(group_id, callback=callback_function)
        ...
        all_params = ['group_id', 'page', 'per_page', 'filters', 'sort_by', 'sort_direction']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_group_customers" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'group_id' is set
        if ('group_id' not in params) or (params['group_id'] is None):
            raise ValueError("Missing the required parameter `group_id` when calling `get_group_customers`")
