...
class CategoriesApi(object):
    ...
    def get_category_thumbnail(self, category_id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_category_thumbnail(category_id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_category_thumbnail_with_http_info(category_id, **kwargs)
        else:
            (data) = self.get_category_thumbnail_with_http_info(category_id, **kwargs)
            return data

    def get_category_thumbnail_with_http_info(self, category_id, **kwargs):
        """
        Get thumbnail of a category
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_category_thumbnail_with_http_info(category_id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_category_thumbnail" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'category_id' is set
        if ('category_id' not in params) or (params['category_id'] is None):
            raise ValueError("Missing the required parameter `category_id` when calling `get_category_thumbnail`")


        collection_formats = {}

        resource_path = '/categories/{category_id}/thumbnail'.replace('{format}', 'json')
        path_params = {}
        if 'category_id' in params:
            path_params['category_id'] = params['category_id']
